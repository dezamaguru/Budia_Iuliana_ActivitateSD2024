...
typedef struct CafeneauaMea Cafenea; //inlocuieste struct Cafenea cu aliasul Cafenea
...
void inserareFinal(Nod** lista, Cafenea c) {
	if ((*lista) ==  NULL) {
		...
		Nod* curent = (*lista);
			...
			n->info = initializareCafenea(c.nume, c.nrLocuri, c.suprafata); //deep copy
			n->next = NULL;
			curent->next = n;
	}
}
...
void main() {
	...
	Cafenea cafenea4 = initializareCafenea("Sb", 23, 43);
	...
	printf("\nCafeneaua cu dens minima: %s", numeMinim);
