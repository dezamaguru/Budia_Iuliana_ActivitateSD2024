...
void inserareInceput(ListaDubla* listaDubla, Cafenea cafenea) { // ListaDubla e pointer pt ca vom modifica lista
    Nod* nou = malloc(sizeof(Nod));  //le alocam in heap
    nou->anterior = NULL;
    nou->cafenea = cafenea;
...
void afisareLista(ListaDubla listaDubla) { //fara pointer => nu modificam lista cand o afisam
    //parcurgem lista
...
int NrTotalLocuri(ListaDubla lista) {//parcurgere inversa
    if (lista.cap == NULL) {
        return 0;
    }
    int nrLocuri = 0;
